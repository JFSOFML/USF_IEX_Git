""" Import streamlit for frontend interface"""
...
if page == "Overview":
    ...
    st.divider()
    st.subheader(
        "Neural Network Implementation with PyTorch and TensorFlow on the MNIST Dataset"
    )
    ...
    makes it an ideal starting point for anyone interested in learning about neural networks\
          and deep learning frameworks such as PyTorch and TensorFlow.
    By working with MNIST, beginners can build a solid foundation in machine learning,\
...
The MNIST (Modified National Institute of Standards and Technology)\
 dataset is a large collection of handwritten digits that is commonly\
used for training various image processing systems.\
 It was created by Yann LeCun, Corinna Cortes, and Christopher Burges.\
The dataset is a subset of a larger set available from NIST,\
 and it was reprocessed to create the MNIST collection.
...
Release: The MNIST dataset was released in the 1990s\
 and quickly became a benchmark for evaluating\
...
"""
    ...
    st.write(
        """This project provides a comprehensive overview of Neural Networks\
          using both PyTorch and TensorFlow on the MNIST dataset.
        It showcases the key steps involved in building, training,\
      and evaluating neural networks with these two powerful frameworks,\
...
if page == "Pytorch":
    ...
    It consists of 60,000 training images \
    ...
    st.write(
        """
    Data loaders are created to feed the data to the model in batches.\
    This helps in efficient training and reduces overfitting by shuffling the data.
    """)

    st.subheader("4. Initializing the Model")
    st.write(
        """
    A neural network model is defined with two fully connected layers.\
    The first layer has 512 neurons with ReLU activation,\
    and the second layer has 10 neurons for each digit class (0-9).
    """)
    ...
    The model is trained for 10 epochs. In each epoch,\
          the model's parameters are updated to minimize the loss.\
...
if page == "Tensorflow":
    ...
    The MNIST dataset is loaded using the TensorFlow library.\
      The images are normalized by dividing the pixel values by\
          255.0 to scale them between 0 and 1.
    """
    )
    ...
    st.write(
        """
    Five sample images from the MNIST dataset are displayed to show the\
         type of data being used. Each image is shown with its corresponding label.
    """
    )
    st.image(
    ...
    The model is trained for 6 epochs on the training data.\
          The training process includes adjusting\
              the model parameters to minimize the loss function.
    """
    )
    st.image(
    ...
    After training, the model is evaluated on the test data to measure its performance.\
          The test loss and accuracy are computed and displayed.
    """
    )
    ...
    st.write(
        """
    The training history, including the training loss and accuracy over epochs,\
          is plotted to visualize the model's learning progress.
    """
    )
    st.image(
...
if page == "Tensorboard":
    ...
    st.write(
        "This section displays three TensorBoard visualizations from the MNIST classification project."
    )

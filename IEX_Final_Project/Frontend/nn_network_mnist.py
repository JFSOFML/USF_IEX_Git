...
if project == "Ames Housing Analysis":
    ...
    **R^2 Score**:
    ...
    with st.expander("Learn more about ensemble learning techniques"):
        ...
        ### Voting:
        Voting is an ensemble method where multiple models vote on the output, \
            and the most common prediction is selected.
        """
        )
...
if project == "Titanic Survival Prediction":
    ...
    st.write(
        "This section will cover the analysis and predictions based on the \
            Titanic dataset."
    )
    ...
    with st.expander("Titanic Overview"):
        st.write(
            """
        This project focuses on predicting the survival of passengers on the \
            Titanic using machine learning techniques.\
              Below are the key steps and processes involved:

        ### Data Preprocessing & Exploratory Data Analysis (EDA)
        - **Loading Data:** Imported the train, test, and gender submission \
            datasets.
        - **Feature Engineering:**
            - Converted the 'Sex' column into a binary format: 0 for male and 1 \
                for female.
            - Created binary indicators for passenger classes: FirstClass, \
                SecondClass, and ThirdClass.
        - **Handling Missing Values:** Filled missing 'Age' values with the \
            mean age from the dataset.
        - **Data Merging:** Merged test data with the gender submission data \
            on 'PassengerId' to create a unified test set.


        - **Visualization:** Used various visualization techniques to explore \
            data distributions and correlations,\
              helping to understand the underlying patterns and relationships \
                in the data.
        ...
        st.write(
            """
        ### Model Training
        - **Feature Selection:** Selected features such as 'Age', \
            'Sex_binary', 'FirstClass', 'SecondClass', 'ThirdClass', \
                'SibSp', 'Parch', and 'Fare' for model training.
        - **Data Normalization:** Applied `StandardScaler` to normalize \
            the features, ensuring that each feature contributed equally\
                  to the model.
        - **Model Implementation:** Used a pipeline to streamline the \
            process, integrating data normalization and the `LinearSVC` \
                model.
        - **Training:** Fitted the model on the training data to learn \
            the patterns associated with survival.

        ### Model Evaluation
        - **Prediction:** Used the trained model to make predictions \
            on the test data.
        - **Accuracy Assessment:** Evaluated the model's performance \
    ...
    with st.expander("Detailed Explanation of Ensemble Learning Techniques"):
        ...
        st.write(
            """
        **Why?**: We import necessary libraries and functions to handle data, \
            preprocess it, and build and evaluate machine learning models.
        """
        )
        ...
        st.write(
            """
        **Why?**: Load the Titanic dataset from CSV files into pandas DataFrames \
        ...
        st.write(
            """
        **Why?**: Fill missing values in 'Age' and 'Fare' with the mean values \
            to ensure no gaps in the data. Convert the 'Sex' column to a binary \
                format (0 for male, 1 for female) to use it in machine learning \
                    models.
        """
        )
        ...
        st.write(
            """
        **Why?**: Select relevant features for training the models and set up \
            the labels (target variable) for prediction. This separates the \
                input variables (features) from the output variable (label).
        """
        )
        ...
        st.write(
            """
        **Why?**: Standardize the features to ensure they all have the \
            same scale. This helps many machine learning algorithms \
                perform better.
        """
        )
        ...
        **Why?**: Define multiple ensemble models:
        - **Bagging**: Combines multiple decision trees to reduce \
            overfitting.
        - **Boosting**: Combines weak learners (shallow trees) to create \
            a strong learner.
        - **Stacking**: Uses multiple models and combines their outputs \
            with a final estimator.
        - **Voting**: Combines predictions from different models and \
        ...
        st.write(
            """
        **Why?**: Train each model on the training data and evaluate \
            their performance on the test data. Print the accuracy to \
                compare how well each model predicts survival.
        """
        )
...
if project == "MNIST Digit Classification":
    ...
    if mnist_page == "Overview":
        ...
        The MNIST (Modified National Institute of Standards and Technology) \
            dataset is a large collection of handwritten digits that is \
                commonly used for training various image processing systems.\
                      It was created by Yann LeCun, Corinna Cortes, and \
                        Christopher Burges. The dataset is a subset of a \
                            larger set available from NIST, and it was \
                                reprocessed to create the MNIST collection.
        """
        )
        ...
        st.write(
            """This project provides the steps of setting up a CNN using \
                both PyTorch and TensorFlow on the MNIST dataset.
        It showcases the key steps involved in building, training, and \
            evaluating neural networks with these two powerful frameworks,
        offering a hands-on approach to understanding the fundamentals \
            of deep learning."""
        )
        ...
        st.write(
            """
        This project demonstrates a basic neural network for classifying \
        ...
        The MNIST dataset is loaded using the torchvision library. \
            It consists of 60,000 training images and 10,000 testing \
        ...
        st.write(
            """
        Data loaders are created to feed the data to the model \
            in batches. This helps in efficient training and \
                reduces overfitting by shuffling the data.
        """
        )
        ...
        st.write(
            """
        The neural network model is defined with two fully \
            connected layers. The first layer has 512 neurons\
                  with ReLU activation, and the second layer \
                    has 10 neurons for each digit class (0-9).
        """
        )
        ...
        The cross-entropy loss function is used to measure the \
            performance of the model. The Adam optimizer is \
        ...
        st.write(
            """
        The model is trained for 10 epochs. In each epoch, \
            the model's parameters are updated to minimize the \
                loss. The training loop involves the following steps:
        - Zero the gradients
        ...
        st.write(
            """
        Five sample images from the MNIST dataset are displayed \
            to show the type of data being used. Each image is \
                shown with its corresponding label.
        """
        )
        st.image(
        ...
        - **Dense Layer**: A fully connected layer with 128 neurons\
              and ReLU activation.
        - **Dropout Layer**: A dropout layer with a 20% dropout rate\
        ...
        After training, the model is evaluated on the test \
            data to measure its performance. The test loss \
                and accuracy are computed and displayed.
        """
        )
        ...
        st.write(
            """
        The training history, including the training loss \
            and accuracy over epochs, is plotted to visualize\
